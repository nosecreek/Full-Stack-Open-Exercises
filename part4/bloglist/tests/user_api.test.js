...
const initialUsers = [
  {
    ...
    name: 'Dustin',
    passwordHash:
      '$2b$10$L7ydxixZktHdGu6iWRW6lOFUVX5hzOm/ezEVfCzYkOHo.k57gBcau',
    __v: 0
  },
  {
    ...
    name: 'Steve',
    passwordHash:
      '$2b$10$N1pForNpqQbBSn.VFAUWsOoW.x496OJUl/LLZWhEN0Mz1sYL6mZDu',
    __v: 0
  },
  {
    ...
    name: 'Joe',
    passwordHash:
      '$2b$10$qp1gXg.wl1I1vdJBdfv90OedAfIcfiYjMzHXZNvFDQNuLHLgDNjve',
    __v: 0
  }
]
...
describe('POST to create a new user', () => {
  ...
  test('if username is a duplicate, return 400', async () => {
    await api.post('/api/users').send(newDuplicateUser).expect(400)
  })

  test('if username is too short, return 400', async () => {
    await api.post('/api/users').send(newShortUsernameUser).expect(400)
  })

  test('if password is missing, return 400', async () => {
    await api.post('/api/users').send(newNoPasswordUser).expect(400)
  })
})
